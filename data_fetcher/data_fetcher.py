...
CLOUD_AMQP_URL = '''amqp://zjzllsqs:R8I54at6fKBzLo8GjG347CYgEYfvXuoz@sidewinder.rmq.cloudamqp.com/zjzllsqs'''
...
def handle_message(msg):
    ...
    if(not isinstance(task, dict) or
       not 'zpid' in task or task['zpid'] is None):
        return
    ...
    db[PROPERTY_TABLE_NAME].replace_one({'zpid':zpid},property_detail,
                                 upsert = True)

    if FETCH_SIMILAR_PROPERTIES:
        #get its similar property's zpid
        ...
        for zpid in similar_zpids:
            old = db[PROPERTY_TABLE_NAME].find_ond({'zpid':zpid})
            #Don't send task if the record is rencent
